...
from kedro_mlops_tp.pipelines.data_science import create_pipeline as create_ds_pipeline
from kedro_mlops_tp.pipelines.data_science.nodes import split_data
...
def test_split_data(dummy_data, dummy_parameters):
    ...
    CONST = 2
    assert len(X_train) == CONST
    assert len(y_train) == CONST
    assert len(X_test) == 1
    assert len(y_test) == 1
